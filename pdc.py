...
Modified on Thu Mar 31 07:48:00 2022

@author: Andor Horvath
@license: GPLv2
"""

""" VERY IMPORTANT! I have to implement a way to add custom queries to the catalogue
on a temporary basis, or to load another catalogue, or to load a catalogue in a
totally different format. So far the library only handles one MySQL-compatible
connection to one catalogue. """
...
class Repertoire_MySQL:
	...
	def __init__(self, repid, dbhost, dbuser, dbpassword, dbname, description, curs, verb=True):
		self.repid = repid
...
class PDC:
	...
	def __init__(self, dbhost, dbuser, dbpassword, dbname, selected=[0], verb=True):
			...
			if(selected == [0] or a[0] in selected):
				if(a[1] == 1):
					self.rep.append(Repertoire_MySQL(
						a[0], a[2], a[3], a[4], a[5], a[6], cursor_megarepdb, verb=verb))
		megarepdb.close()
		self.main = self.search('mainlist')
		self.full = self.search('listall')
	...
	def valuelist(self, parameter, idy, selected=[0]):
	...
	def pardesc(self, parameter, selected=[0]):
		"""Return a list of all possible values for a given parameter."""
		result = set()
		for a in self.rep:
			if(a.repid in selected or selected == [0]):
				result = result.union(a.pardesc(parameter))
		return list(result)
